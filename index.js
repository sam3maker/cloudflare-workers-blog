...
addEventListener("fetch",t=>{t.respondWith(async function(t){
  ...
  // R2附件上传接口（需要admin鉴权）
  if(method==="POST" && path==="/admin/upload" && typeof BUCKET!=="undefined"){
    try{
      ...
      const publicURL=OPT.r2PublicURL?OPT.r2PublicURL+"/"+key:"/admin/file/"+key;
      return new Response(JSON.stringify({ok:1,url:publicURL,key:key}),{headers:CORS_HEADERS,status:200});
    }catch(err){
      return new Response(JSON.stringify({ok:0,msg:err.message}),{headers:CORS_HEADERS,status:400});
    }
  }
  ...
  // R2附件访问路由（公开访问上传的文件）
  if(method==="GET" && path.startsWith("/admin/file/") && typeof BUCKET!=="undefined"){
    try{
      const key=path.replace("/admin/file/","");
