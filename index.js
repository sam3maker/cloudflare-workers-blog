...
addEventListener("fetch",t=>{t.respondWith(async function(t){
  ...
  if(method==="GET" && path.startsWith("/admin/file/")){
    try{
      ...
      headers.set("Content-Disposition","attachment; filename=\""+(obj.name||"download")+"\"");
  ...
  if(method==="POST" && path==="/admin/upload"){
    try{
      ...
      const uint8=new Uint8Array(arrayBuf);let binary="";for(let i=0;i<uint8.length;i++)binary+=String.fromCharCode(uint8[i]);const base64=btoa(binary);
